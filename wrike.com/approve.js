...
(function() {
  ...
  WrikeTaskFilters = (function() {
    ...
    WrikeTaskFilters.prototype.renderFlags = function() {
      ...
      if ($('#' + this.cfg.taistFiltersContainerId).length) return;
      ...
      for (_ in states) {
        ...
        if (this.filter === flag.text()) flag.addClass(self.cfg.flagCheckedClass);
  ...
  WrikeTaskApprover = (function() {
    ...
    WrikeTaskApprover.prototype.stateFromTitle = function() {
      ...
      if (!(m != null ? m[1] : void 0)) return 'initial';
      for (stateName in states) {
        state = states[stateName];
        if (state.titleTag === m[1]) return stateName;
      }
    };
