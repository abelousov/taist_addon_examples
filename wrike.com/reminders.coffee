->
  ...
  class Reminder
    ...
    _loadReminderData: (callback) ->
      ...
      taistApi.userData.get "defaultSettings", (error, defaultSettingsData) =>
        @_defaultSettings = defaultSettingsData
        taistApi.userData.get @_task.data.id, (error, existingReminderData) =>
    ...
    _save: (newEvent, calendarId, callback) ->
      ...
      taistApi.userData.set @_task.data.id, {eventId: newEvent.id, calendarId}, =>
        taistApi.userData.set "defaultSettings", @_defaultSettings, -> callback()
    ...
    _accessEvent: (method, params, callback) ->
      @_api.events[method](params).execute (eventOrResponse) ->
        if eventOrResponse.error?
          taistApi.error "couldn't #{method} event: ", params, eventOrResponse.error
        else
