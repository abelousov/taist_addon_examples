...
(function() {
  var docMap, getColorOfStatus, getColorOfStatusByHash, getCompanyName, getUserSettings, needdraw, prevHash, setUserSettings, start, startChangeHash, startDraw, startDrawButton, startDrawCollorPicker, userSettings, utils, waitChangeHash, waitDraw, waitDrawButton, waitDrawCollorPicker;
  utils = null;
  userSettings = [];
  ...
  startDraw = function() {
    var color, column, hash, i, index, jrow, row, _i, _j, _len, _len1, _ref, _ref1;
    i = 0;
    index = null;
    ...
    if (index != null) {
      _ref1 = $('table.b-document-table>tbody').find('tr');
    ...
    return needdraw = false;
  };
  waitDraw = function() {
    return utils.wait.repeat((function() {
      return $('table.b-document-table').find('td:not([style*="background"])').length !== 0 || needdraw;
    }), startDraw);
  };
  startChangeHash = function() {
    prevHash = $(location).attr('hash');
    return needdraw = true;
  };
  waitChangeHash = function() {
    return utils.wait.repeat((function() {
      return $(location).attr('hash') !== prevHash;
    }), startChangeHash);
  };
  ...
  startDrawButton = function() {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      newSaveButton.bind('click', function() {
        var currentColor, currentDocType, input, inputObj, key, value, _j, _len1, _ref1;
