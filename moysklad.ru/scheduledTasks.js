...
(function() {
  var Calendar, CalendarSettings, EditableCalendarOptions, InDocCalendar, baseCalendarOptions, calendarDisplay, calendarFactory, calendarStorage, drawGeneralCalendarTab, drawSettingsTab, entryPoint, eventEditDialog, getCompanyKey, init, loadJsLibraries, moyskladAPI, moyskladUtils, projectsStorage, renderGeneralCalendar, start, taistApi, taskStorage, waitDrawDocumentCalendar;
  taistApi = null;
  moyskladUtils = null;
  ...
  init = function(callback) {
    ...
    return getCompanyKey(function(companyKey) {
      taistApi.companyData.setCompanyKey(companyKey);
      return calendarStorage.init(function() {
        return taskStorage.init(function() {
          return projectsStorage.init(function() {
            return calendarDisplay.init(callback);
          });
        });
      });
    });
  };
  getCompanyKey = function(cb) {
    return $.get(moyskladAPI + "/MyCompany/list/?count=1").done(function(xml) {
      return cb($('accountUuid:first', xml).text());
    });
  };
  drawGeneralCalendarTab = function() {
    return moyskladUtils.topMenu.addMenuItemWithoutSubItems('Календарь', renderGeneralCalendar);
  };
  drawSettingsTab = function() {
    return moyskladUtils.topMenu.addMenuItemWithoutSubItems('Календарь', function(mainContainer) {
      return (new CalendarSettings()).render(mainContainer);
    });
  };
  ...
  renderGeneralCalendar = function(mainContainer) {
    ...
    return calendarFactory.createReadOnly(calendarElement);
  };
  CalendarSettings = (function() {
    function CalendarSettings() {}

    CalendarSettings.prototype._calendarsList = null;

    CalendarSettings.prototype.render = function(mainContainer) {
      var addButton, header, saveButton, settingsContainer;
      ...
      settingsContainer.append(header, this._calendarsList, saveButton);
      return mainContainer.append(settingsContainer);
    };

    CalendarSettings.prototype._renderCalendars = function() {
      var calendar, j, len, ref, results;
      if (this._calendarsList == null) {
        this._calendarsList = $("<div class=\"addonScheduledTasks-settingsCalendarsList\"></div>");
      } else {
        this._calendarsList.empty();
      }
      ref = calendarStorage.getOrderedCalendars();
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {
        calendar = ref[j];
        results.push(this._renderCalendar(calendar));
      }
      return results;
    };

    CalendarSettings.prototype._renderCalendar = function(calendar) {
      var calendarContainer, calendarDeleteLink, calendarNameInput;
      ...
      calendarNameInput.on('input', function() {
        return calendar.name = calendarNameInput.val();
      });
      calendarDeleteLink = moyskladUtils.uiPrimitives.button({
        ...
        click: (function(_this) {
          return function() {
            return _this._delete(calendar);
          };
        })(this)
      });
      calendarContainer.append(calendarNameInput, calendarDeleteLink);
      return this._calendarsList.append(calendarContainer);
    };
    ...
    CalendarSettings.prototype._delete = function(calendar) {
      ...
      return this._renderCalendars();
    };

    CalendarSettings.prototype._save = function() {
      return calendarStorage.save((function(_this) {
        return function() {
          return _this._renderCalendars();
        };
      })(this));
    };
  ...
  InDocCalendar = (function() {
    ...
    InDocCalendar.prototype._renderTaskStructure = function() {
      ...
      this._calendarToggleButton = moyskladUtils.uiPrimitives.button({
        ...
        classes: "addonScheduledTasks-inDocCalendarCloseLink draggable",
        click: (function(_this) {
          return function() {
      ...
      taskListHeader = $('<h2 class="addonScheduledTasks-taskListHeader draggable">Задачи: </h2>');
    ...
    InDocCalendar.prototype._renderContainers = function() {
      var containersExistAlready, getPositionKey, position, prevEl, renderDiv, rootWrapper, rootWrapperClass, tabPanelWrapper;
      ...
      getPositionKey = function() {
        ...
        return "calendarPosition" + hash;
      };
      position = taistApi.localStorage.get(getPositionKey());
      if (position) {
        this._taskListContainer.css({
          left: position.left,
          top: position.top
        });
      }
      if (!containersExistAlready) {
        ...
        return this._taskListContainer.draggable({
          handle: '.draggable',
          stop: function(event, ui) {
            var key;
            taistApi.log(ui.position);
            key = location.hash.replace(/\/.+$/, '');
            return taistApi.localStorage.set(getPositionKey(), ui.position);
          }
        });
      }
    };
    ...
    InDocCalendar.prototype._getTasksByCalendarNames = function(entityId) {
      var calendar, calendarTasks, j, k, len, len1, orderedTasks, ref, task, tasksByCalendars;
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        calendar = ref[j];
        calendarTasks = null;
        for (k = 0, len1 = orderedTasks.length; k < len1; k++) {
          task = orderedTasks[k];
  ...
  eventEditDialog = {
    ...
    _createDialog: function(isNew) {
      var calendarSelect, dialog, nameInput, timeEnd, timeStart, title;
      title = isNew ? "Создание задачи" : "Изменение задачи";
      dialog = this._renderDialog(title);
      nameInput = dialog.find('input.taskName');
      timeStart = dialog.find('input.timeStart');
      timeEnd = dialog.find('input.timeEnd');
    ...
    _fillCalendarSelect: function(event, calendarSelect) {
      var calendar, j, len, ref, results, selected;
      ref = calendarStorage.getOrderedCalendars();
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {
        calendar = ref[j];
        selected = event.calendarId === calendar.id ? "selected" : "";
        results.push(calendarSelect.append($("<option value=\"" + calendar.id + "\" " + selected + ">" + calendar.name + "</option>")));
      }
      return results;
    },
    _showDialog: function(dialogElement, okHandler) {
      var onClose;
      ...
      return dialogElement.dialog({
        ...
        close: function() {
          return dialogElement.remove();
        }
      });
    },
    _renderDialog: function(title) {
      return $("<div title=\"" + title + "\">\n  <div>\n    <div class=\"addonScheduledTasks-inDocEditDialogInput\">\n      <label>Календарь:\n        <select></select>\n      </label>\n\n    </div>\n\n    <div class=\"addonScheduledTasks-inDocEditDialogInput\">\n      <label>\n        Название:\n        <input name=\"taskName\" type=\"text\" class=\"taskName\"/>\n      </label>\n    </div>\n\n    <div class=\"addonScheduledTasks-inDocEditDialogInput\">\n      <label>\n        Время:\n        <input name=\"timeStart\" type=\"text\" class=\"timeStart\">\n        -\n        <input name=\"timeEnd\" type=\"text\" class=\"timeEnd\">\n      </label>\n    </div>\n\n  </div>");
    }
  };
  calendarFactory = {
    createReadOnly: function(calendarContainer) {
      return this._createCalendar(calendarContainer, (function(_this) {
        return function() {
          return baseCalendarOptions;
        };
      })(this));
    },
    createEditable: function(entityId, calendarContainer, onUpdate) {
      return this._createCalendar(calendarContainer, function(calendar) {
        return new EditableCalendarOptions(entityId, calendar, onUpdate);
      });
    },
    _createCalendar: function(calendarContainer, createCalendarOptionsCallback) {
  ...
  baseCalendarOptions = {
    ...
    eventAfterRender: function(event, element, view) {
      ...
      return moyskladUtils.api.getProjectUuid(event.entityId, event.taskData.entityType, function(projectUuid) {
        var color;
        if (projectUuid) {
          color = projectsStorage.getProjectColor(projectUuid);
          return element.addClass('bg' + color);
        }
      });
    },
    events: function(start, end, unusedTimezone, callback) {
      var event, eventsList, fn, i, j, len, ts;
      ts = new Date().getTime();
      i = 0;
      ...
      fn = function(event) {
        ...
        return event.url = location.href.replace(location.hash, '#' + entityHashPath);
      };
      for (j = 0, len = eventsList.length; j < len; j++) {
        event = eventsList[j];
        fn(event);
      }
  ...
  EditableCalendarOptions = (function() {
    EditableCalendarOptions.prototype._calendar = null;

    EditableCalendarOptions.prototype._onUpdate = null;
    ...
    EditableCalendarOptions.prototype._bindPublicMethods = function() {
      var propName, propValue, results, self;
      self = this;
      results = [];
      for (propName in self) {
        propValue = self[propName];
        if (propName[0] !== "_" && typeof propValue === "function") {
          results.push((function(propName, propValue) {
            return self[propName] = function() {
    ...
    EditableCalendarOptions.prototype._updateTaskFromEvent = function(event) {
      return taskStorage.update(event, (function(_this) {
        return function() {
    ...
    EditableCalendarOptions.prototype._decorateFreshlyCreatedEvent = function(event) {
      var eventBelongsToCurrentEntity;
      event.className += " addonScheduledTasks-calendarEvent";
      eventBelongsToCurrentEntity = event.entityId === this._entityId;
    ...
    EditableCalendarOptions.prototype.selectHelper = true;

    EditableCalendarOptions.prototype.editable = true;

    EditableCalendarOptions.prototype.eventDrop = function(event) {
      return this._updateTaskFromEvent(event);
    };

    EditableCalendarOptions.prototype.eventResize = function(event) {
      return this._updateTaskFromEvent(event);
    };

    EditableCalendarOptions.prototype.select = function(start, end) {
      eventEditDialog.edit({
        start: start,
        end: end,
        entityId: this._entityId
      }, true, (function(_this) {
        return function(newEventData) {
          return taskStorage.create(newEventData, function(newEvent) {
            _this._decorateFreshlyCreatedEvent(newEvent);
            _this._calendar["do"]('renderEvent', newEvent);
            return _this._onUpdate();
          });
        };
      })(this));
    ...
    EditableCalendarOptions.prototype.eventClick = function(event, jsEvent) {
    ...
    EditableCalendarOptions.prototype.eventRender = function(event, domElement) {
      if (event.editable) {
        domElement.append(this._createDeleteLink(event));
      }
      return domElement;
    };

    EditableCalendarOptions.prototype.events = function(start, end, timezone, callback) {
      return baseCalendarOptions.events(start, end, timezone, (function(_this) {
        return function(eventsList) {
          var event, j, len;
          for (j = 0, len = eventsList.length; j < len; j++) {
            event = eventsList[j];
            _this._decorateFreshlyCreatedEvent(event);
          }
          return callback(eventsList);
        };
      })(this));
    ...
    EditableCalendarOptions.prototype._createDeleteLink = function(event) {
      var deleteLink;
        ...
        return function() {
          if (window.confirm("Удалить задачу \"" + event.title + "\"?")) {
            return taskStorage["delete"](event, function() {
              _this._calendar["do"]('removeEvents', function(taskToConfirmRemoval) {
    ...
    return EditableCalendarOptions;

  })();
  Calendar = (function() {
    Calendar.prototype._screenBorderMargin = 20;

    Calendar.prototype._calendarContainer = null;

    Calendar.prototype._calendarControl = null;

    function Calendar(_calendarContainer) {
      this._calendarContainer = _calendarContainer;
    }

    Calendar.prototype.render = function(options) {
      var calendarDisplayFlags;
      calendarDisplayFlags = this._renderCalendarDisplayFlags();
      this._calendarControl = $('<div></div>');
      this._calendarContainer.append(calendarDisplayFlags, this._calendarControl);
      this["do"](options);
      return this._adjustToScreenHeight();
    };
    ...
    Calendar.prototype._renderCalendarDisplayFlags = function() {
      var calendar, flagsContainer, fn, j, len, ref, self;
      self = this;
      ...
      ref = calendarStorage.getOrderedCalendars();
      fn = function(calendar) {
        var checkbox, displayFlag, isVisible;
        ...
        checkbox.click(function() {
          return calendarDisplay.setCalendarVisibility(calendar.id, checkbox.prop("checked"), function() {
            return self._redraw();
          });
        });
        return flagsContainer.append(displayFlag);
      };
      for (j = 0, len = ref.length; j < len; j++) {
        calendar = ref[j];
        fn(calendar);
    ...
    Calendar.prototype._adjustToScreenHeight = function() {
      var maxCalendarHeight;
      maxCalendarHeight = window.innerHeight - 2 * this._screenBorderMargin;
      if (this._calendarContainer.height() > maxCalendarHeight) {
        return this["do"]('option', 'height', maxCalendarHeight);
      }
    };

    Calendar.prototype._redraw = function() {
      return this["do"]('refetchEvents');
    };
  ...
  calendarStorage = {
    ...
    init: function(callback) {
      return taistApi.companyData.get(this._calendarsKey, (function(_this) {
        return function(calendars) {
          var defaultCalendar;
          _this._calendars = calendars;
          if (_this._calendars != null) {
            return callback();
          } else {
            _this._calendars = [];
            defaultCalendar = _this.create();
    ...
    getOrderedCalendars: function() {
      return this._calendars.slice().sort(function(cal1, cal2) {
        return cal1.name.localeCompare(cal2.name);
      });
    },
    save: function(callback) {
      return taistApi.companyData.set(this._calendarsKey, this._calendars, callback);
    },
    create: function() {
      var newCalendar;
      newCalendar = {
    ...
    "delete": function(calendar) {
      return this._calendars.splice(this._calendars.indexOf(calendar), 1);
    },
  ...
  projectsStorage = {
    ...
    _backgroundColors: [],
    init: function(callback) {
      this._backgroundColors = this._backgroundColorsString.split(',');
      return taistApi.companyData.get(this._projectsDataKey, (function(_this) {
        return function(projectsData) {
          _this._projectsData = projectsData != null ? projectsData : {};
          console.log(_this._projectsData);
          return callback();
        };
      })(this));
    },
    _save: function() {
      return taistApi.companyData.set(this._projectsDataKey, this._projectsData, function() {});
    },
    getProjectColor: function(uuid) {
      var color, colorNum, ref;
      if (!((ref = this._projectsData[uuid]) != null ? ref.color : void 0)) {
        colorNum = Object.keys(this._projectsData).length;
        color = this._backgroundColors[colorNum % this._backgroundColors.length];
        this._projectsData[uuid] = {
          color: color
        };
        this._save();
      }
      return this._projectsData[uuid].color;
    }
  };
  taskStorage = {
    ...
    _updateTask: function(taskData, eventData, callback) {
      ...
      taskData.calendarId = eventData.calendarId;
    ...
    getTasksForTimeRange: function(start, end) {
      ...
      for (entityId in this._tasksData) {
        filteredEntityEvents = (this.getEntityEvents(entityId)).filter((function(_this) {
          return function(event) {
            return (_this._eventFitsTimeRange(event, start, end)) && calendarDisplay.calendarIsVisible(event.calendarId);
          };
        })(this));
    ...
    _eventFitsTimeRange: function(event, start, end) {
      return (!event.start.isBefore(start)) && (!event.end.isAfter(end));
    },
    getEntityEvents: function(entityId) {
      var j, len, rawTask, ref, results;
      ref = this._getEntityTasks(entityId);
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {
        rawTask = ref[j];
        results.push(this._constructEvent(rawTask, entityId));
      }
      return results;
    },
    _getEntityTasks: function(entityId) {
      var base;
      if ((base = this._tasksData)[entityId] == null) {
        base[entityId] = [];
      }
  ...
  calendarDisplay = {
    ...
    init: function(callback) {
      return taistApi.userData.get(this._hiddenCalendarsKey, (function(_this) {
        return function(hiddenCalendars) {
          _this._hiddenCalendars = hiddenCalendars != null ? hiddenCalendars : {};
          return callback();
        };
      })(this));
    },
    calendarIsVisible: function(calendarId) {
      return !this._hiddenCalendars[calendarId];
    },
    setCalendarVisibility: function(calendarId, isVisible, callback) {
      this._hiddenCalendars[calendarId] = !isVisible;
      return taistApi.userData.set(this._hiddenCalendarsKey, this._hiddenCalendars, callback);
    }
  };
  moyskladUtils = {
    ...
    api: {
      _uuidToProjectUuid: {},
      _resolveProjectUuidCallbacks: function(uuid) {
        var callback, results;
        results = [];
        while (this._uuidToProjectUuid[uuid].callbacks.length > 0) {
          callback = this._uuidToProjectUuid[uuid].callbacks.pop();
          results.push(callback(this._uuidToProjectUuid[uuid].projectUuid));
        }
        return results;
      },
      getProjectUuid: function(uuid, type, callback) {
        ...
        if (!this._uuidToProjectUuid[uuid]) {
          this._uuidToProjectUuid[uuid] = {
            callbacks: [],
            isInProgress: true,
            projectUuid: null
          };
        ...
        if (this._uuidToProjectUuid[uuid].isInProgress) {
          return this._uuidToProjectUuid[uuid].callbacks.push(callback);
        } else {
          return typeof callback === "function" ? callback(this._uuidToProjectUuid[uuid].projectUuid) : void 0;
        }
      }
    },
    topMenu: {
      ...
      _createTopMenuItem: function(itemName, clickHandler) {
        return taistApi.wait.elementRender('.topMenu tr', (function(_this) {
          return function(topMenu) {
            var newMenuItem;
            _this._prepareNativeMenuForAddingCustomMenuItems();
      ...
      _renderNewTopMenuItem: function(topMenu, itemName) {
        ...
        menuItem = $("<td class=\"" + this._topMenuItemClass + " " + this._customMenuItemClass + "\"><span title=\"" + itemName + "\" class=\"lognex-SpanHyperlink my-style tutorial-chapter-tab\" tabindex=\"0\"><a><span class=\"text\">" + itemName + "</span></a></span></td>");
    ...
    uiPrimitives: {
      ...
      _getButtonIcon: function() {
        return $("<img src=\"https://online.moysklad.ru/app/admin/clear.cache.gif\" style=\"width:14px;background:url(https://online.moysklad.ru/app/admin/5DED07A8C93EDC2459C97B778717F84B.cache.png) no-repeat ;\" border=\"0\">");
      },
      _getButtonDecoratorsByTypes: function() {
        return {
          button: {
            template: function(options) {
              return "<div role=\"button\" class=\"b-popup-button b-popup-button-enabled " + options.classes + "\" >\n<table><colgroup><col></colgroup><tbody><tr><td></td><td><span class=\"text\">" + options.caption + "</span></td></tr></tbody></table>\n                                                      </div>";
            },
            addIcon: function(button, icon) {
              icon.addClass("icon");
              icon.height(14);
              return button.find('td:first-child').append(icon);
            },
            stylesByActions: {
              add: {
          ...
          link: {
            template: function(options) {
              return "<div class=\"b-link-button " + options.classes + "\"><span class=\"image-link-button-text\" >" + options.caption + "</span></div>";
            },
            addIcon: function(link, icon) {
              icon.height(13);
              return link.prepend(icon);
            },
            stylesByActions: {
              "delete": {
      ...
      button: function(options) {
        var button, buttonDecorator, icon, ref, styles;
        buttonDecorator = this._getButtonDecoratorsByTypes()[(ref = options.type) != null ? ref : "button"];
        button = $(buttonDecorator.template(options));
        styles = buttonDecorator.stylesByActions[options.action];
        if (styles != null) {
          if (styles.iconPosition != null) {
            icon = this._getButtonIcon();
            buttonDecorator.addIcon(button, icon);
            icon.css("background-position", styles.iconPosition);
          }
        }
        button.addClass((styles != null ? styles : buttonDecorator.stylesByActions["default"]).classes);
        if (options.type === 'link') {
          $('span', button).click(options.click);
        } else {
          button.click(options.click);
        }
        return button;
